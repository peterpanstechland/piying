...
export class LegProcessor {
  ...
  private updateJumpMetrics(landmarks: PoseLandmarks): void {
    ...
    // Only update baseline if we are NOT flying/jumping/squatting to avoid dragging baseline
    const isSquatting = this.state.overallIntent === LegIntent.SQUATTING
    if (!this.isInFlyingState && !isSquatting && Math.abs(this.hipVelocity) < 0.01) {
  ...
  private determineOverallIntent(): LegIntent {
    ...
    // 4. 下蹲检测 (Squat Detection)
    // 髋部相对于基准线显著下降 (baselineHipY - currentHipY < squatThreshold, note threshold is negative)
    // Pixi Y is down, so Squat Y > Standing Y. Baseline - Current < 0.
    if (this.baselineHipY !== null) {
      const hipHeightChange = this.baselineHipY - this.lastHipY
      if (hipHeightChange < squatThreshold) {
        return LegIntent.SQUATTING
      }
    }
